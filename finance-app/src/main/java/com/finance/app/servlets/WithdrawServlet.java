...
public class WithdrawServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
        ...
        try {
            ...
            if (currentBalance.compareTo(amount) < 0) {
                session.getTransaction().rollback();
                resp.setStatus(400);
                resp.getWriter().write("{\"error\":\"Insufficient balance\"}");
                return;
            }
            ...
            tx.setAmount(amount);
            tx.setType("withdraw");
            tx.setDescription(desc != null ? desc : "Withdrawal");
            ...
            resp.setStatus(200);
            resp.getWriter().write(gson.toJson(new WithdrawResponse(true, "Withdrawal successful", newBalance.doubleValue(), tx.getId())));
        } catch (Exception e) {
    ...
    static class WithdrawResponse {
        ...
        WithdrawResponse(boolean success, String message, double newBalance, long transactionId) {
